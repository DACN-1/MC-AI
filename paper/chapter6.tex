...
a contrastive one? & \textbf{No}, in this regime. CLIP collects $4.3$ dirt per
episode where the matched LLaVA cell is at the floor ($0.0$); the gap is a
read-out effect, since a FiLM head recovers CLIP-level collection from the same
LLaVA features (Sections~\ref{sec:exp-task}, \ref{sec:exp-film}). \\
...
CLIP + lang shifts (\texttt{attack} $-48$~pp, $h=-1.04$); both LLaVA cells and the
CLIP control are flat (Section~\ref{sec:exp-cond}). \\
...
diverge from $4.3$ to $0$ dirt in rollout (Section~\ref{sec:exp-offline}). \\
\hline
\end{tabular}
