...
dirt collection, CLIP digs in every episode with the prompt and in eight of ten
without it, while both LLaVA anchor cells sit at the floor; on conditioning, CLIP
with language shifts its \texttt{attack} rate by $48$ percentage points between
the empty and on-task prompts, an effect an order of magnitude larger than
anything either LLaVA cell produces. For a practitioner who wants a controllable
short-horizon agent on a small budget, the result is encouraging: the cheaper
$0.43$B contrastive encoder, paired with a $4.6$M-parameter head, is the more
effective choice here \emph{under this minimal head}, not the $7$B autoregressive
one. Set against the contractor it imitates, though, even the best agent is modest
in absolute terms: measured from the recordings, the demonstrator collects $18.3$
dirt per thousand steps, so CLIP\,+\,lang's $4.3$ is about a quarter of the expert
rate --- far above the random floor of zero, but a reminder that the comparison
this study makes cleanly is between cells, not against mastery.
...
even ranks CLIP $-$ lang first --- yet they diverge from $4.3$ to $0$ dirt in
rollout (Section~\ref{sec:exp-offline}). The head-to-head verdict is therefore a
...
the two prompts are perfectly correlated with two biomes, and the pooled LLaVA
text half is image-entangled --- its tokens attend to the frame inside the decoder
--- so the $99.7\%$ is largely a prompt--image confound rather than a clean prompt
axis. The FiLM probe is the decisive check: a head that multiplicatively gates the
features on that very text vector, strictly more expressive than the concatenating
baseline, still does not condition (Section~\ref{sec:exp-film}). If a usable prompt
axis were present, a gating head would exploit it; that it cannot indicates there
is none to exploit, and head capacity is therefore not the limiting factor. The
contrast with CLIP isolates what is: CLIP's text encoder is physically separate
from its image encoder, so its prompt embedding is an image-independent axis that
moves when, and only when, the prompt changes, and the head learns to ride it.
LLaVA's final-layer text positions, mean-pooled, fold the prompt and the frame
together, so changing only the prompt at a fixed scene barely moves the
representation. The bottleneck is thus upstream of the head, and the remedies that
follow are representational rather than architectural on the head side: pooling the
prompt from a position that is not image-saturated --- the final text token rather
than the mean --- or lightly fine-tuning the backbone so the text pathway carries a
separable signal. A counterfactual probe that swaps the prompt at a fixed frame
would measure the confound directly, and is the cleanest next test
...
the CLIP\,+\,lang agent's \texttt{attack} key, from $84.9\%$ under the empty prompt
to $37.0\%$ --- a drop in precisely the action that chopping requires. A natural
...
attack-heavy, and the empty-prompt policy ($84.9\%$) faithfully reproduces that
...
the suppressed prompted one completes a chop. The $48$-point shift thus shows that
...
in-environment comparisons rest on ten episodes per cell; although MineRL is
seed-deterministic, so each run reproduces exactly, those ten episodes are still a
small sample over ten distinct world seeds. The per-step firing rates that carry
the conditioning result are estimated from $\sim\!10^4$ observations and are
robust, but the per-episode task metrics are noisy over that small sample, so the
within-CLIP prompt-versus-no-prompt difference is only suggestive and the
cross-backbone task gap is the one task comparison I treat as reliable. This
sampling caveat has a counterpart on the training side: each cell is a single
training run at one seed, so the headline effects --- the $-48$-point conditioning
shift, the symmetric LLaVA flats, the cross-backbone dirt gap --- carry no
across-training-seed error bars, and head initialisation or data ordering cannot
be formally excluded as contributors. The per-step effect sizes are large enough
($|h|>1$ for the conditioning shift) that seed noise is an unlikely explanation,
but replicating each cell across several training seeds is the most important
robustness check this study leaves open. The quantitative \emph{task} story, in
turn, rests on a single working task: chop is at the floor for every cell, so dirt
collection is the only task on which the agents produce a completion metric at
all, with the chop environment contributing the per-step conditioning shift and
the qualitative targeting result but no scalar separation. Second, the study fixes the
...
measure of goal pursuit: the no-prompt cells dig comparable amounts in the chop
and dirt environments (Table~\ref{tab:task}), so an absolute dirt count conflates
goal-directed digging with incidental ground-attacking, and it is the
prompt-driven \emph{lift} between environments, not the raw count, that cleanly
reflects task-following. Finally, the LLaVA no-prompt control is not a perfect
...
\cite{kumar2022finetuningdistortpretrainedfeatures, simchowitz2025pitfalls}. The
cleanest diagnostic that should precede either is a counterfactual probe:
re-encode a fixed set of frames under each prompt and measure how far the pooled
text representation moves, quantifying directly how much of the $99.7\%$
decodability of Section~\ref{sec:exp-probe} is a usable prompt axis rather than the
prompt--image confound.
