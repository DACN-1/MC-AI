...
class _Combined640EnvSpec(HumanControlEnvSpec):
    ...
    # NB: no create_observables() override — HumanControlEnvSpec already exposes a
    # FlatInventoryObservation, so obs['inventory'] is available for free. The
    # agent ignores it (run_rollout feeds only obs['pov']); run_rollout now LOGS
    # it as the reward-independent task signal (RewardForCollectingItems is broken
    # here: dirt/logs visibly enter the HUD with total_reward=0).
