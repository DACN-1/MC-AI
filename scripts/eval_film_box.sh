#!/bin/bash
# Box-side native eval for the FiLM (v2) LLaVA head (llava_fix_v2.md).
# Same as eval_fixAB_box.sh: native run_rollout per condition, IDENTICAL decode
# flags (symmetric with the paper cells). Resumable. Output -> llava_lang_film.
ulimit -n 1048576
export JAVA_HOME=/usr/lib/jvm/java-8-openjdk-amd64
cd /workspace/r1-va
source /workspace/venv/bin/activate
mkdir -p logs logs/minerl_watchers evaluations/paper
HEAD=output/llava_combined_lang_stride4_film/model_best.pt
TAG=llava_lang_film
